...
int main()
{
    ...
    // plt::title("SGD: $f(x,y) = 0.05x^{2} + y^{2}$");
    // plt::legend();
    // plt::save("SGD_function_gradient_descent.png");
    // plt::cla();

    plt::plot(param_history[0], param_history[1], {{"color", "red"}, {"marker", "o"}, {"linestyle", "--"}, {"label", "$f(x, y) = \\frac{1}{20}x^{2} + y^{2}$"}});
    plt::plot(vector<double>(1, 0), vector<double>(1, 0), "b+");
    plt::title("Momentum: $f(x,y) = 0.05x^{2} + y^{2}$");
    plt::legend();
    plt::save("Momentum_function_gradient_descent.png");
    plt::cla();
